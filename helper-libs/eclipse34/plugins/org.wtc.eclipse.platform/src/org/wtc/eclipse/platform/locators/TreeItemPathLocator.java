...
/**
 * Describes a node in a tree path to a TreeItem. TreeItemPathLocator objects can be
 * chained to create a path to a TreeItem. This object can be used instead of
 * TreeItemLocator if a finer-grained match of TreeItems is to be found. For example, if a
 * tree exists:
 *
 * <pre>
 *     + Root_1
 *         + MiddleUnique
 *             + ChildUnique <-- *select this [A]*
 *         + MiddleSame
 *             + Child
 *             + Child
 *             + Child
 ...
 *     </pre>
 *
 * <p>If we want to select the node identified with "select this [A]" then there are two
 * ways:</p>
 *
 * <ol>
 *   <li>
 *     <pre>
 *     IUIContext ui = getUI();
 *     ui.click(new TreeItemLocator("Root_1/MiddleUnique/ChildUnique"));
 *     </pre>
 *   </li>
 *   <li>
 *     <pre>
 *     IUIContext ui = getUI();
 *     TreeItemPathLocator nodePath
 *         = new TreeItemPathLocator("Root_1",
 *               new TreeItemPathLocator("MiddleUnique",
 *                   new TreeItemPathLocator("ChildUnique")));
 *     nodePath.findAndClick(ui);
 *     </pre>
 *   </li>
 * </ol>
 *
 ...
 *     TreeItemPathLocator nodePath
 *         = new TreeItemPathLocator("Root_1",
 *               new TreeItemPathLocator("MiddleSame", 1
 *                   new TreeItemPathLocator("Child", 2)));
 *     nodePath.findAndClick(ui);
 *     </pre>
 *   </li>
 * </ol>
...
public class TreeItemPathLocator {
     ...
     *       TreeItemPathLocator)
     */
    public TreeItemPathLocator(String labelPattern, int index, TreeItemPathLocator child) {
     ...
     *       TreeItemPathLocator)
     */
    public TreeItemPathLocator(String labelPattern, Tree parentTree, TreeItemPathLocator child) {
     ...
     *       TreeItemPathLocator)
     */
    public TreeItemPathLocator(int index, Tree parentTree, TreeItemPathLocator child) {
     ...
     *
     * @param  labelPattern  - The text label as seen on the TreeItem or a regular
     *                       expression that identifies the TreeItem by label. This
     *                       parameter is optional. If <code>null</code> then find the
     *                       first TreeItem matching the union of the other criteria
     * @param  index         - The index of the TreeItem in its parent TreeItem (or Tree
     *                       if this is a root node). This parameter is optional. If -1
     *                       then find the first TreeItem matching the union of the other
     *                       criteria
     * @param  parentTree    - The Tree widget containing this TreeItem. This parameter is
     *                       optional. If <code>null</code> then find the first TreeItem
     *                       matching the union of the other criteria (the first TreeItem
     *                       as searched in all visible Tree widgets that match the other
     *                       criteria will be found)
     * @param  child         - The description of the child node to click, or <code>
     *                       null</code> if this is the leaf node in the tree path that is
     *                       to be clicked.
     */
    public TreeItemPathLocator(String labelPattern,
                               int index,
    ...
    /**
     * Walk the tree path described by this node: Click this node to select it then, if
     * there is a child tree path node, expand this node and recurse into the child
     * TreeItemPathLocator
     *
     * @param   ui  - Driver for UI-generated input
     * @throws  WidgetSearchException  - If the TreeItem described by this tree item path
     *                                 cannot be found for any reason
     */
    public void findAndClick(IUIContext ui) throws WidgetSearchException {
        findAndClick(ui, null);
    }
    ...
    private TreeItem findAndClick(IUIContext ui, TreeItem parent) throws WidgetSearchException {
        ...
        String message = MessageFormat.format("DID NOT FIND A TREE NODE FOR ITEM <{0}>", //$NON-NLS-1$
                                              new Object[] { toString() });
     ...
     * Walk the tree path described by this node: Click this node to select it then, if
     * there is a child tree path node, expand this node and recurse into the child
     * TreeItemPathLocator. When the leaf node is found, context click the node and
     * execute the menu action with the given menu item label
     *
     * @param   ui        - Driver for UI-generated input
     * @param   menuItem  - The path to the menu item to select (can be a regular
    ...
    /**
     * @param  parent  - Used to debugging purposes only
     */
     ...
     */
    private class TreeItemByIndexLocator extends SWTWidgetLocator implements IItemLocator {
         ...
         */
        public TreeItemByIndexLocator(int index,
                                      String labelPattern,
                                      TreeItem parentItem,
                                      Tree parentTree) {
        ...
        /**
         * @return  TreeItem - The TreeItem that was found and clicked with this widget
         *          locator
         */
        ...
        public boolean matches(Object obj) {
            ...
            if ((_foundItem == null) && (obj != null) && (TreeItem.class.isAssignableFrom(obj.getClass()))) {
        ...
        /**
         * @return  boolean - True if the given value is exactly equal to the expected
         *          label or the given value matches a regex expression compiled from the
         *          expected label pattern; False otherwise
         */
